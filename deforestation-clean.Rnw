...
my.countries <- c("Angola", "Tanzania")
...
country.data <- Reduce(merge, sapply(c("API_NY.GDP.PCAP.CD_DS2_en_excel_v2_5795829.xlsx", "API_NE.EXP.GNFS.ZS_DS2_en_excel_v2_5728793.xlsx", "API_NV.AGR.TOTL.ZS_DS2_en_excel_v2_5728821.xlsx", "API_SP.POP.1564.TO_DS2_en_excel_v2_5731200.xlsx"), function(x) {
    ...
    temp.dt.2 <- melt(temp.dt.1, id.vars = "Country Code", variable.name = "raw.year", value.name = meta.data)
    temp.dt.2[, raw.year := as.numeric(as.character(temp.dt.2$raw.year)), ]
    ## Create a prober name, based on the tree letter code
    temp.dt.2[, country := iso.3166$String[match(`Country Code`, iso.3166$String.code)], ]
    ## Drop the three letter code.
    temp.dt.2[, `Country Code` := NULL, ]
    ## Drop cases that has missing data on the identifier variables.
...
base.url.generic <- "https://storage.googleapis.com/earthenginepartners-hansen/GFC-"

base.url <- paste0(base.url.generic, version, "/")

## file.name.prefix <- "Hansen_GFC-2021-v1.9_"
...
url.from.point.f <- function(lon, lat, layer){
    ...
    my.file <- paste0(file.name.prefix, layer, "_", lat.corner, "_", lon.corner, ".tif")    
    return(my.file)
}
...
for(i in 1:length(dhs.grid)){
    ...
    for(my.layer in c("treecover2000", "lossyear")){
        ...
        for(j in 1:length(my.files)){
	        ...
	        globallivingconditions::my.log.f(paste(Sys.time(), "polygon number", i, "j", j, "will download this url:", my.urls[j], "and save it as", file.and.path), "deforestation.log")
...
tile_extent_grid <- function(r, nx, ny) {
  ...
  for (i in seq_len(nx)) {
    for (j in seq_len(ny)) {
      tile_ext <- raster::extent(xbreaks[i], xbreaks[i+1], ybreaks[j], ybreaks[j+1])
...
my.mosaic <- function(...){ raster::mosaic(..., fun = mean, tolerance = 1, na.rm = TRUE) }

my.g <- function(i) {
    ## Create the large raster
    globallivingconditions::my.log.f(paste0(Sys.time(), " Starting to aggregate polygon number ", i, "."))
    my.raster <- load.raster.from.disk(dhs.grid[i], deforestation.path = deforestation.path)
    globallivingconditions::my.log.f(paste0(Sys.time(), " Raster loaded for polygon number ", i, "."))    
    ...
    ## Tile raster into approximately equal areas (note that some areas can be empty).

    ## TODO adjust number of tiles to size of the raster.
    ...
    if(my.nx > 1){
        globallivingconditions::my.log.f(paste(Sys.time(), "using", my.nx, "by", my.nx, "tiles for polygon number", i, "of size", my.size))

        tiles_ext <- tile_extent_grid(my.raster, nx = my.nx, ny = my.nx)
        ## On 8 cores this give 4x4 = 16 tiles, might be too many, but OTOH seems to require less RAM.
        ## tiles.with.data <- sapply(tiles_ext, function(mask) { raster::crop(my.raster, mask) })

        ## ## Apply terra::aggregate() in parallel
        ## aggregated_tiles <- future.apply::future_lapply(tiles.with.data, function(tile) {
        ##     raster::aggregate(tile, 75)
        ## }, future.seed=TRUE) ## 5 GB

        ## Apply crop and terra::aggregate() in parallel
        aggregated_tiles <- future.apply::future_lapply(tiles_ext, function(mask) {
        ...
        globallivingconditions::my.log.f(paste(Sys.time(), "Not parallelising the small polygon number", i, "of size", my.size))
    ...
    globallivingconditions::my.log.f(paste0(Sys.time(), " Finished aggregating polygon number ", i, "."))
    return(my.small)
}
## ## parallelise if on unix
## if(.Platform$OS.type == "unix") {
##     ## leave at least 25 GB per region to aggregate. Reduce this when working with smaller regions.
##     n.cores <- min(parallelly::availableCores(method = c("Slurm", "system"))-1,
##                    floor(as.vector(benchmarkme::get_ram() / 1E9) / 25))
##     globallivingconditions::my.log.f(paste0(Sys.time(), " n.cores=", n.cores))
##     foo <- parallel::mclapply(these.regions, my.g, mc.cores = n.cores)
##     globallivingconditions::my.log.f(paste(Sys.time(), "Finished reading in rasters."))
## } else {
...
future::plan("multicore", workers = n.cores) ## Assume multicore support
options(future.globals.maxSize = 5e9)

## foo <- sapply(these.regions, my.g)
foo <- parallel::mclapply(these.regions, my.g, mc.cores = ceiling(n.cores/2))

globallivingconditions::my.log.f(paste(Sys.time(), "Finished aggregating rasters."))

## ## Convert to terra SpatRasters, so that we can use mosaic() which is faster than merge()
## terra_list <- lapply(foo, terra::rast)

## ## bar <- Reduce(my.f, foo)
## bar <- do.call(terra::mosaic, terra_list)

bar <- do.call(my.mosaic, foo)

globallivingconditions::my.log.f(paste(Sys.time(), "Finished with the mosaic."))
...
if(.Platform$OS.type == "unix") {
    ## leave at least 30 GB per region to aggregate. Reduce this when working with smaller regions.
    n.cores <- min(parallelly::availableCores(method = c("Slurm", "system"))-1,
...
dep.subset.f <- function(var.name){
    ...
    ## If some region where present in foo, but not in output.dt.2, explain why.
    
    if(length(output.dt.2[is.na(average.years.of.education.for.the.1956.1976.cohort) == FALSE, unique(superClusterID), ]) < length(unique(foo$superClusterID))){
        these <- which(is.na(match(unique(foo$superClusterID), output.dt.2[is.na(average.years.of.education.for.the.1956.1976.cohort) == FALSE, unique(superClusterID), ])))
        globallivingconditions::my.log.f(paste(Sys.time(), "Some regions lacked enough data on education for the cohort: ", paste(unique(foo$superClusterID)[these], collapse = ", ")))
        save(foo, file = "foo.RData")
    }
    if(length(my.dt.with.temporal.loss[is.na(dep.var) == FALSE, unique(superClusterID), ]) < length(unique(my.dt.with.temporal.loss$superClusterID))){
        these <- which(is.na(match(unique(my.dt.with.temporal.loss$superClusterID), output.dt.2[is.na(dep.var) == FALSE, unique(superClusterID), ])))
        globallivingconditions::my.log.f(paste(Sys.time(), "Some regions lacked enough data on deprivation: ", paste(unique(unique(my.dt.with.temporal.loss$superClusterID)[these], collapse = ", "))))
        save(foo, file = "foo.RData")
    }
    if(length(my.dt.with.temporal.loss[is.na(loss.at.year.of.interview) == FALSE, unique(superClusterID), ]) < length(unique(my.dt.with.temporal.loss$superClusterID))){
        these <- which(is.na(match(unique(my.dt.with.temporal.loss$superClusterID), output.dt.2[is.na(loss.at.year.of.interview) == FALSE, unique(superClusterID), ])))
        globallivingconditions::my.log.f(paste(Sys.time(), "Some regions lacked enough data on loss.at.year.of.interview: ", paste(unique(unique(my.dt.with.temporal.loss$superClusterID)[these], collapse = ", "))))
        save(foo, file = "foo.RData")
    }
...
colnames(foo) <- c("DF", "GDP", "FC", "year", "area", "EduY", "D.", "Non D.", "p.D.")
