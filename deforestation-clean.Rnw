...
required.packages <- c("httr", "data.table", "car", "sp", "raster", "devtools", "purrr", "doParallel", "snow", "viridisLite", "geosphere", "parallel", "foreach", "tmap", "terra", "sf", "lme4", "parallelly")
...
Taking the mean value is easy enough. To see that the mean carries some signal, plot the mean values of all regions in the first country in the grid, and compare that with the raw version. First plot the raw data, see figure \vref{fig:raw-data-senegal}.
...
bar <- Reduce(my.f, foo)
plot(bar, col = rev(terrain.colors(101)), zlim = c(0, 100), main="")
lines(get.border(substring(names(dhs.grid)[1], 1, 3)))
@ 
...
if(.Platform$OS.type == "unix") {
    foo <- unlist(parallel::mclapply(these.regions, temp.f, mc.cores = n.cores))
} else {
    foo <- sapply(these.regions, temp.f)
}
