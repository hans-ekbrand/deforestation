...
my.mosaic <- function(...){ raster::mosaic(..., fun = mean, tolerance = 1) }
...
my.g <- function(i) {
    ...
    ## Tile raster into approximately equal areas

    ## TODO adjust number of tiles to size of the raster.
    tiles_ext <- tile_extent_grid(my.raster, nx = floor(n.cores/2), ny = floor(n.cores/2))
