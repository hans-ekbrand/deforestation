...
## parallelise if on unix
if(.Platform$OS.type == "unix") {
    ## leave at least 15.5 GB per region to aggregate
    n.cores <- min(parallelly::availableCores(method = c("Slurm", "system"))-1,
                   floor(as.vector(benchmarkme::get_ram() / 1E9) / 15.5))
    globallivingconditions::my.log.f(paste(Sys.time(), "n.cores=", n.cores))
...
globallivingconditions::my.log.f(paste(Sys.time(), "Finished converting rasters to terra."))
...
library(xtable)
print(xtable(foo[1:42, ], digits = c(0, 2, 0, rep(0, 3), 2, rep(0, 2), 2), label = "tab:data", caption = "Abbreviations: Deforestation ('DF'), Forest Class ('FC'), Average years of education for the 1956 1976 cohort ('EduY') Deprived ('D.'), and Not.Deprived ('Non D.') for an area in Ghana."), include.rownames = FALSE, booktabs = TRUE)
@ 
