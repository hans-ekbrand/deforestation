...
my.countries <- c("Angola", "Tanzania", "Morocco", "Egypt", "Tunisia", "Mauretania", "South Africa", "Nigeria", "Congo Democratic Republic")
...
my.g <- function(i, n.cores.for.this.instance) {
    ...
    my.raster <- load.raster.from.disk(dhs.grid[i],
                           deforestation.path = deforestation.path)
    globallivingconditions::my.log.f(paste0(Sys.time(),
             " my.g(): Raster loaded for polygon number ", i, "."))    
...
temp.f <- function(i) {
    globallivingconditions::my.log.f(paste0(Sys.time(),
                         " taking the mean of area with index ", i))    
    ...
    if(length(attr(my.values, which = "condition")) > 0){
        globallivingconditions::my.log.f(paste0(Sys.time(),
                         " temp.f() failed to read area with index ", i))
        return(NA)
    }
...
easy.ones <- data.table::rbindlist(foreach::foreach(
           ...
           my.data.table <- try(data.table::rbindlist(temporal.loss.for.multiple.areas.from.a.single.grid.cell(multiple.areas.one.tiff[[j]], deforestation.path = deforestation.path)), silent = TRUE)
           if(length(attr(my.data.table, which = "condition")) > 0){
...
save(data.sets, my.dt.with.temporal.loss, file = "data.sets_and_my.dt.with.temporal.loss.RData")
rm(my.dt)
gc(verbose=FALSE)
