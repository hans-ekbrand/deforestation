...
if(file.exists("/mimer/NOBACKUP/groups/globalpoverty1/hans")){
  ...
  the.largest.half <- my.order[1:(length(area.sizes)/2-1)]
  the.smallest.half <- my.order[(length(area.sizes)/2):length(area.sizes)]  
  areas.by.size.classes <- c(the.largest.half, the.smallest.half)
}
