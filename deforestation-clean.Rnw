...
dep.subset.f <- function(var.name){
    ...
    if(length(unique(foo$superClusterID)) > length(unique(output.dt.2$superClusterID))){
        these <- which(is.na(match(unique(foo$superClusterID), unique(output.dt.2$superClusterID))))
        globallivingconditions::my.log.f(paste(Sys.time(), "Some regions lacked enough data: ", paste(unique(foo$superClusterID)[these], collapse = ", ")))
