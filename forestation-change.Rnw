...
process_one_tile_one_year <- function(tile_file, polys_sf, tile_to_poly, poly_id_col = "poly_id") {
...
compute_forest_share_for_year <- function(polys_sf,
  ...
  res_list <- future_lapply(tile_files, function(f) {
    process_one_tile_one_year(tile_file = f, polys_sf = polys_sf, tile_to_poly, poly_id_col = poly_id_col)
  }, future.seed = TRUE)
...
my.sf$forest.coverage.change <- out$net_change_pp

@ 

<<forest-change, fig.cap = "Forest Coverage Change for two example countries: Benin and Nigeria.">>=
library(ggplot2)
